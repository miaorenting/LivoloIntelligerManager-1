...
import livolo.com.livolointelligermanager.mode.WifiBean;
...
//        if (null != wifiinfo) {
//            wifimanager.disableNetwork(wifiinfo.getNetworkId());//让一个网络连接失效
//            wifimanager.setWifiEnabled(true);//让一个网络链接有效
//            wifimanager.disconnect();
//        }
        boolean result = false;
        if (config.networkId > 0) {
            result = wifimanager.enableNetwork(config.networkId, true);
            wifimanager.updateNetwork(config);// 更新一个网络连接
    ...
    public static void addNetWIFI(String ssid, String password, ScanResult device,Context context) {
        ...
        WifiSupport.addNetWork(wifiConfiguration, context);
    }

    private static void myChangeWIFI(String ssid,WifiManager manager,Context context){
        WifiConfiguration wificonfig = WifiSupport.isExsits(ssid,context);
